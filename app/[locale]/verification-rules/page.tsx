...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return pageMetadata({
    ...
    title: dict.legal.verificationRules.title,
    description: dict.legal.verificationRules.subtitle,
  });
}

export default async function VerificationRulesPage({ params }: Props) {
  const { locale } = await params;
  const dict = await getDictionary(isLocale(locale) ? (locale as Locale) : 'uz');
  const doc = dict.legal.verificationRules;
