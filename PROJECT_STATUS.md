...
| Testing / CI | 55% | CI runs **lint (ESLint next/core-web-vitals) + typecheck + tests + build** on `npm ci` + committed lockfile. Focused **Vitest** suite (52 tests) covers Click/Payme callback verification, amount/currency validation, commission math + the net↔gross withdrawal conversion + available-balance scenarios, and `confirmDonation` idempotency; no RLS/integration tests yet. |
