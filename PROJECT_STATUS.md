...
> **Branch:** main · **Latest at sync:** the homepage **Picked** section is now ranked by **funds actually raised** (highest first; ties → donor count → most recent donation → newest campaign), shows **only ACTIVE** campaigns, and **never** shows a campaign with 0 raised — filtered and sorted in Postgres on existing indexes, no admin "featured" flag, no migration ([Picked Campaigns](#picked-campaigns--homepage-curation-funds-raised-ranking)) — on top of admin withdrawal review now showing the NET transfer amount prominently (was rounded gross "10 ming") via a single `payoutBreakdown()` source of truth + net-amount notifications (#63) — on top of the withdrawal-workflow audit (funded-campaign RPC #61) + validation/preview polish + display-rounding fix (`formatMoney` rounded net 9,600 → "10 ming"; now exact "9 600 so'm") + simplified withdrawal UX + net-display payout model (no migration) + Payme (#48 pending) + Click
>
...
| Testing / CI | 55% | CI runs **lint (ESLint next/core-web-vitals) + typecheck + tests + build** on `npm ci` + committed lockfile. Focused **Vitest** suite (**143 tests across 12 files**) covers Click/Payme callback verification, amount/currency validation, commission math + the net↔gross withdrawal conversion + available-balance/net-display scenarios (10k/100k/1M, multiple/pending withdrawals, refunds), `confirmDonation` idempotency, campaign limits, drafts + draft cleanup, Instagram video parsing, token cipher, responsive grid, and the **Picked-campaigns funds-raised ranking**; no RLS/integration tests yet. |
...
- **Creation limits (goal + duration):** goal **≤ 1,000,000,000 UZS** (and > 0; existing 100k minimum kept) and duration **≤ 60 days** (end date after today, ≤ 60 days from the creation date — `created_at` when editing). Single source of truth in `lib/campaign-limits.ts` (`MAX_GOAL_AMOUNT`, `MAX_DURATION_DAYS`, `isGoalWithinLimit`, `isDurationWithinLimit`, `todayISO`, `maxDeadlineISO`; unit-tested in `__tests__/campaign-limits.test.ts`, 10). Enforced at **three layers**: (1) the create + edit forms (localized inline errors via `t('limits.goalMax')`/`t('limits.durationMax')`, `max` on the number input, `min`/`max` on the date picker, and helper hints `limits.goalHint`/`limits.durationHint`); (2) the **server** `/api/campaigns/create` route (zod `max(MAX_GOAL_AMOUNT)` + `isDurationWithinLimit` → 422); (3) the DB trigger **`guard_campaign_limits`** (`supabase/campaign-limits.sql`) — the real server-side backstop for **edits**, which are written client-direct under RLS (no server route). The trigger fires on write only, checks a cap on UPDATE only when that field changes, and exempts admins + the donation-credit bypass, so **existing campaigns are never modified or blocked** on unrelated edits. `locales/{uz,ru,en}/common.json` (`limits.*`). **No schema/column change**; the trigger migration is required for server-side enforcement of the edit path.

### Campaign Video (optional, Instagram external link)
- **What:** A creator can attach **one optional** Instagram **Post or Reel** link to a campaign (create + edit). We deliberately **do not host or embed video** (storage/bandwidth + no third-party iframe/script); the campaign page shows a provider-neutral **"▶ Watch campaign video"** button (with a small **"Opens in Instagram"** caption) that opens the post in a **new tab** (`target="_blank" rel="noopener noreferrer"`). No video ⇒ **nothing renders** (no placeholder). Existing campaigns keep `video_url = NULL` and are unaffected.
- **Where:** `lib/video/instagram.ts` (provider: validate/normalize) + `lib/video/index.ts` (provider-agnostic `parseVideoUrl`/`isValidVideoUrl`/`normalizeVideoUrl` + registry) — unit-tested in `__tests__/video-instagram.test.ts` (12). UI: `components/campaigns/VideoLink.tsx` (external-link button, renders in `CampaignDetail` after the story), the create field in `CreateCampaignForm` (auto-saved into the draft + validated on submit), and add/replace/remove in `EditCampaignForm`. Server: `/api/campaigns/create` re-validates + stores the **canonical** URL only. `locales/{uz,ru,en}/common.json` (`video.*`).
- **Validation rules:** accepts only `https://www.instagram.com/(p|reel)/<shortcode>/` on the `instagram.com` / `www.` / `m.` hosts (http, missing-www, `/reels/`, trailing path and query strings are normalized to the canonical `https://www.instagram.com/(p|reel)/<shortcode>/`). Rejects non-Instagram hosts, lookalikes (`instagram.com.evil.com`), the `instagr.am` shortener, malformed URLs, profile URLs, stories, and explore links. Shortcode is bounded `^[A-Za-z0-9_-]{1,64}$`. Validated **client-side** (instant message: "Invalid Instagram link") **and authoritatively server-side** (create route rejects 422; the client string is never stored — only the normalized canonical permalink is).
- **Security:** no iframe, no embed script, **no CSP change** (the link is an ordinary `<a>` to `instagram.com`). The `href` is the canonical permalink built from a validated shortcode (never raw user HTML — no `dangerouslySetInnerHTML`); `rel="noopener noreferrer"` isolates the opened tab.
- **DB:** `supabase/campaign-video.sql` — `alter table … add column if not exists video_url text` on **campaigns** + **campaign_drafts** (nullable, no default, no backfill; idempotent). `video_url` is an ordinary owner-editable content column (guard trigger only locks status/current_amount/donors_count/views), so the existing edit path adds/replaces/removes it with no trigger/RLS change.
- **Future providers:** add `lib/video/<provider>.ts` + one registry line in `lib/video/index.ts`, plus a provider→caption entry (`CAPTION_KEY`) in `VideoLink.tsx`; the button text itself is provider-neutral. The rest of the campaign code (forms, route, storage, link component) is provider-agnostic and needs no change. No CSP change (external links only).
- **Status:** ⏳ Code-complete; **pending migration `campaign-video.sql` applied in Supabase.** Until applied, the create/edit selects that reference `video_url` would error — apply before deploy.

### Campaign Drafts & Auto-save
- **What:** A creator can save an **unfinished** campaign and resume it later. The create form **auto-saves** (debounced 1.2s) into a private `campaign_drafts` row and shows a live save status (**Saving… / Saved / Saved at HH:MM**); a manual **Save as draft** button forces an immediate save. Images are uploaded to the `campaign-images` bucket **on select** (not at submit), so their URLs are stored on the draft and **uploads stay associated across sessions**. Drafts are **private** (owner-only RLS; never appear in any public listing — those all query `public.campaigns`). A dedicated **Drafts section** in My Campaigns lists each draft with **Continue editing**, **Submit for review** (only when complete), and **Delete**; each shows last-edited time + a ready/incomplete hint. Opening the create page with an existing draft offers **Continue Draft** or **Start New Campaign** (`?draft=<id>` deep-links straight into a specific draft). **Submitting converts the draft into a normal Pending Review campaign** by posting through the **existing** `/api/campaigns/create` route (Turnstile + KYC gate + publish trigger all still apply) and then deleting the draft row — **no campaign-creation logic is duplicated**.
- **Why a separate table (not `status='draft'`):** `public.campaigns` has NOT NULL `title/slug/description/goal_amount` + a UNIQUE slug, so a half-filled form cannot live there. `campaign_drafts` holds nullable partial state; the real campaign is still created the one existing way on submit.
- **Where:** `supabase/campaign-drafts.sql` (table + owner-only RLS + `updated_at` trigger), `lib/drafts.ts` (pure helpers: `hasDraftContent`, `draftColumns`, `draftToCreatePayload`/`isDraftSubmittable`, `draftDisplayTitle` — unit-tested in `__tests__/drafts.test.ts`), `components/campaigns/{CreateCampaignForm,CreateCampaignClient}.tsx`, `components/profile/MyCampaigns.tsx` (Drafts section), `app/[locale]/campaigns/create/page.tsx` + `app/[locale]/profile/campaigns/page.tsx` (load drafts, graceful), `locales/{uz,ru,en}/common.json` (`draft.*`). Draft CRUD runs client-side under RLS (mirrors `campaign_updates`).
- **Security:** the `campaign_drafts` table is queried **only** on the owner's own create + My-Campaigns surfaces (all `user_id`-scoped); it is never touched by home / listings / search / explore / featured / admin moderation / donations / sitemap / SEO metadata. RLS is owner-only for select/insert/update/delete (`user_id = auth.uid()`) with **no admin or anon policy**, so anonymous users see nothing and admins cannot read others' drafts (nor does any admin API query the table). Submission uses a client single-flight lock **and** deletes the draft only after the create route returns OK, so a repeated/Enter-key submit cannot create duplicate campaigns and a failed submit keeps the draft.
- **Abandoned-draft cleanup (implemented):** a daily sweep deletes drafts untouched for **30 days** (`updated_at < now()-30d`) and garbage-collects the images they uploaded to the `campaign-images` bucket — but **only images referenced by nothing else**. Architecture: a reusable service `lib/drafts/cleanup.ts` (`cleanupAbandonedDrafts(client, opts)`) usable by the cron route, a manual admin trigger, or a maintenance script; it never throws for normal data conditions and returns a summary `{ draftsScanned, draftsDeleted, imagesDeleted, imagesSkipped, imageFailures, draftFailures, durationMs }`. **Safety:** an image is removed only when it lives in our bucket (URL shape verified via `storagePathFromPublicUrl`), is referenced by **no campaign** (`image_url` or `images[]`) and **no other draft**, and every reference check succeeded — any match or any query error ⇒ the image is **kept** (never delete under uncertainty). Published/pending/rejected campaigns and their images are never touched (a different table). **No orphans:** a draft row is deleted only after every image it intended to delete was actually removed; if a storage delete fails the row is **retained** for the next run. **Failure handling:** a failed image delete or draft-row delete is counted in the summary and the sweep continues. **Endpoint:** `GET /api/cron/cleanup-drafts` — `nodejs`, `force-dynamic`, gated by `verifyCronSecret` (Bearer `CRON_SECRET`, constant-time, fail-closed in prod). **Schedule:** `vercel.json` cron `0 3 * * *` (daily 03:00 UTC). **Logging:** summary-only (`console.log('[cron/cleanup-drafts]', {counts})`) — no user data or secrets. **Env:** requires `CRON_SECRET` + `SUPABASE_SERVICE_ROLE_KEY` (service-role client bypasses RLS to see all users' drafts/campaigns for the reference checks). **Perf:** processes ≤`DEFAULT_DRAFT_SCAN_LIMIT` (500) oldest drafts per run; per image up to 4 short indexed lookups (2 on `campaigns`, 2 on `campaign_drafts`) — a large backlog drains over successive daily runs rather than one long invocation. **Rollback:** delete the `vercel.json` cron entry (and/or unset `CRON_SECRET` → endpoint fails closed) to halt the sweep instantly; the service is pure-additive and touches no creation/publication path, so removing it leaves the drafts feature unchanged. Deletions are irreversible (rows + storage objects), so consider a dry-run/soft-delete before widening the window. **Tests:** `__tests__/drafts-cleanup.test.ts` (11) — older/newer than 30d, image shared with another draft, image referenced by a campaign, storage-delete failure, db-delete failure, path extraction.
- **Status:** ⏳ Code-complete; **pending migration `campaign-drafts.sql` applied in Supabase.** Degrades gracefully until then — the create/My-Campaigns pages and auto-save all detect a missing table and no-op, so existing campaign creation is unaffected and the feature simply activates once the migration runs. Live E2E (create/edit/auto-save/resume/submit/delete) requires the table applied; the pure mapping/validation logic is covered by 15 unit tests.
...
### Picked Campaigns / Homepage Curation (funds-raised ranking)
- **What:** The homepage's **Picked** section (top of the campaign area, 3-up card grid — `dict.home.featured*`) shows the **3 highest-funded ACTIVE campaigns**, ranked by money actually raised. It is **not** an admin selection and **not** recency-based. "Trending" below it still shows the **top 8 by raised amount** out of the newest 24, minus whatever Picked already used.
- **Qualification (both required):** `status = 'active'` **and** total raised **> 0**. Draft / Pending Review / Rejected / Completed / Funded / Expired / Paused / Cancelled (and any other non-active state) are excluded **by construction** — the query pins `status='active'`, so a campaign that isn't actively collecting can never appear. A **never-funded campaign (0 raised) is never shown**; if no campaign has raised anything the **entire section hides** rather than rendering empty cards.
- **Ranking (highest first):** (1) **total raised** (`campaigns.current_amount`) → (2) **donor count** (`campaigns.donors_count`) → (3) **most recent donation** (latest completed donation) → (4) **newest campaign** (`created_at`).
- **No admin "featured" flag:** there is no `is_featured` column in the schema and **none was introduced**. The section is derived entirely from money raised, so it cannot be curated and it **re-ranks itself as donations land** (within the ISR window, `revalidate = 60`). `current_amount`/`donors_count` are the denormalized totals maintained by the `apply_donation()` trigger — credited on a completed donation, **reversed on refund/failure** — so "raised" is always the live net total, never client-supplied.
- **Where:** `lib/picked-campaigns.ts` (the single source of truth: `getPickedCampaigns`, `PICKED_CAMPAIGN_LIMIT`, plus pure `isPickableCampaign` / `comparePickedCampaigns` / `rankPickedCampaigns`), consumed by `app/[locale]/page.tsx`. Copy in `locales/{uz,ru,en}/common.json` — **keys unchanged** (`home.featuredBadge`/`featuredTitle`/`featuredSubtitle`), values reworded from an admin-selection framing ("Tanlangan Kampaniyalar" / "Featured Campaigns") to a funds-raised one ("Eng Ko'p Yig'ilgan Kampaniyalar" / "Кампании с наибольшим сбором" / "Top Funded Campaigns"). **Card design and section layout are unchanged** — same `CampaignCard` with `featured`, same grid/badge/heading markup.
- **Backend filtering + sorting (performance):** the filter and the first two ranking keys run **in Postgres** (`.eq('status','active').gt('current_amount', 0)` + `.order(current_amount desc).order(donors_count desc).order(created_at desc)`), hitting the existing partial indexes `idx_campaigns_active_raised (current_amount desc) where status='active'` and `idx_campaigns_active_donors` — no full-table scan and no client-side sort of the campaign set. Key 3 (most recent donation) is the one value Postgres doesn't keep on `campaigns`; rather than add a denormalized column (**a migration**), it is resolved by **one extra indexed query against the existing public `campaign_donors` view** — which already restricts to `status='completed'`, is anon-readable and exposes **no donor PII** — and that query only runs when candidates actually **tie on both amount and donor count** (normally never). Query reuse only: no new table, column, RPC, view, or index.
- **Fixed — recency-window bug:** Picked no longer derives from the homepage's "newest 24 active campaigns" fetch, so a **top earner that is no longer recent still shows**. (The old `featured = campaigns.slice(0,3)` could only ever surface recent campaigns; Trending inherits the same 24-row window and was deliberately left as-is.)
- **Defence in depth:** the qualification rule is re-applied client-side in `isPickableCampaign` after the query, so even if the DB predicate were ever weakened a 0-raised or non-active campaign still cannot reach the homepage. Every failure path (query error, exception, missing donation data) returns `[]` or degrades the tiebreak to `created_at` — the homepage never throws.
- **Status:** ✅ Complete. **No migration required.** Typecheck + production build pass; **19 unit tests** in `__tests__/picked-campaigns.test.ts` pin the invariants (raised-desc ordering, 0-raised always excluded, every non-active status rejected even when better funded, each of the 3 tiebreakers in strict precedence, a tie group straddling the 3-card cut, limit/empty handling).
...
- **Financial rule (audited 2026-07-19; net-display model shipped 2026-07-20):** the 4% platform fee is **deducted FROM the gross withdrawal** — the GROSS leaves the balance and the creator receives `amount − fee` (`payout_amount`), enforced by the DB `CHECK (commission_amount + payout_amount = amount)`. The DB semantics are unchanged, but the **creator now thinks entirely in NET so'm**: "Available to withdraw" (dashboard + dialog max + Max button) shows `calcNetPayout(availableGross)` — the exact amount they can request AND receive today — and the amount they type **is the amount the server pays, to the so'm**. The client converts the entered net to the exact gross via `grossForNet()` (`lib/payout.ts`), which inverts the server's `round(gross × 0.04)` formula (net(g) steps by 0/1 per so'm of gross ⇒ every net value is reachable; the smallest matching gross is chosen ⇒ lowest fee; the Max case sends the full gross so the balance drains to exactly 0). **No DB migration needed** — this delivers the net-request UX recommended by the 2026-07-19 audit *without* the atomic-deploy hazard it warned about, because `create_payout_request` still receives a gross ≤ the available balance. The displayed net minimum is the documented round **5,000** (not the technically-derived 4,800 net of the server's gross minimum — a clean familiar number on a trust surface); it is deliberately *stricter* than the server guard (5,000 net → 5,208 gross ≥ 5,000), so `below_minimum` is unreachable for any client-accepted amount and the gates can never disagree. The dialog reconciles live on every keystroke: entered amount = "you receive"; the 4% fee row is informational (labelled "already deducted" — charged on the gross the available figure already excludes, never subtracted from the entered amount again); "Available to withdraw afterwards" = availableNet − entered (0 on Max). Withdrawal history + its details modal lead with the stored net (`payout_amount`) and break it down as gross deducted − fee actually charged (historical 0%/3% rows keep their real rate, never re-derived). Ledger/admin/`financial_summary` are untouched (gross bookkeeping is correct there).
- **Financial-transparency audit (2026-07-20) — canonical answers:** (1) **When platform fees are deducted:** the 4% is computed and stored at request time (`commission_amount = round(amount × 0.04)`), taken FROM the requested gross; ledgered as `platform_fee` when the payout is marked **paid**. Donors are never charged. (2) **When provider fees are deducted:** **never, currently** — `providerFee = 0` on every surface; the `provider_fee` ledger type is reserved but always 0 (no gateway charges one in the live flow yet). (3) **What "Available to withdraw" represents (creator-facing, since the net-display model):** the **NET** = `calcNetPayout(campaign_available_balance)` — the exact amount the creator can request and receive today; the underlying DB value stays gross. **One calculation source (`lib/payout.ts`):** `calcPlatformFee`/`calcNetPayout` (mirror of the server formula), `grossForNet` (its exact inverse), `MIN_WITHDRAWAL` (server gross mirror) / `MIN_WITHDRAWAL_NET` (creator-facing minimum, = 5,000 net), `calcAvailableGross` + `COMMITTED_STATUSES`, and — **the one "Available to withdraw" display formula** — `calcAvailableNet(currentAmount, requests) = calcNetPayout(calcAvailableGross(...))` (+ `netAvailableFromGross` for netting an already-computed gross aggregate) (mirror of `campaign_available_balance`, now shared by the creator withdraw page AND the admin payouts page — the duplicated inline mirrors were removed). The withdraw page derives every figure from these and passes them to the dashboard (`CampaignFinancials`) and the dialog (`CampaignPayouts`); the RPC re-computes authoritatively. **Dashboard reconciles visibly** (see the 2026-07-20 simplification entry below for the current row set — the dashboard was later rebuilt as a NET waterfall: Total donations → Platform fee (total) → Provider fee → Completed withdrawals (net) → Pending withdrawals (net) → **Available to withdraw**, reconciling to `totalDonations = platformFee + providerFee + completedNet + pendingNet + availableNet`). **Terminology unified:** "Available to withdraw" (`dash.availableBalance` = `cf.availableBalance`, identical wording in uz/ru/en) is always the net. Unit-tested (`__tests__/payout.test.ts`, 52 tests): exhaustive net→gross round-trip (every net 1..25,000 pays exactly the typed amount at the lowest gross), min/Max mapping, and the availability matrix (no/one/multiple donations, pending/approved/paid/rejected/cancelled requests, refunds, over-commit floor at 0, full drain to exactly 0).
- **Net "Available to withdraw" consolidated to one formula + admin aligned (2026-07-20):** audited every surface in the verify list (available/max/Max-button/amount/you-receive/remaining/dashboard/financial-summary/history/admin-review/ledger). The **creator surfaces were already net** (the withdraw card, dialog headline/Maximum/Max/amount ceiling, and the per-campaign `CampaignFinancials` summary all render `calcNetPayout(available)`; the gross `campaign_available_balance` RPC is never on a display path) — so a 10,000 gross balance already shows 9,600 to creators. Two changes make the whole app derive that number from **one** formula: (1) introduced `calcAvailableNet` in `lib/payout.ts` as THE display formula and routed the withdraw page through it (was an inline two-step); (2) the **admin financial summary's "Available for withdrawal"** (`financial_summary.available_for_withdrawal`, shown on `/admin/finance` and the printed report) was the only surface still emitting **gross** — it now nets via `netAvailableFromGross` in `lib/finance.ts` (app-layer, no migration, same fee rule), so the admin aggregate means the same NET as the creator figure. **Admin payout *review* stays gross by design** — it verifies a gross request `amount` against the gross pool and already shows the net `payout_amount` per request; netting it would make a valid full-balance request look like an over-withdrawal. **Ledger unchanged** — it already records `withdrawal = −payout_amount` (net) + `platform_fee = −commission` per movement. Verified by 8 new tests (60 total): available = actual net payout at 10k/100k/1M, multiple/pending withdrawals, and post-refund balances; the shown amount always equals `calcNetPayout(gross)` and never over-withdraws.
- **Withdrawal-dialog transparency pass (2026-07-20, same-day follow-up):** the dialog leads with a prominent **Available to withdraw** block (brand-tinted card matching the dashboard tile: label + amount + one-line explanation of where the number comes from — completed donations minus platform fee and previous/pending withdrawals, `dash.availableExplain`); a short **after-submit note** (`dash.afterSubmitNote`: reviewed in 1–3 business days → sent to the card above → notification) states the process at the decision point, since the page's fuller processing card is hidden behind the modal overlay; and the dialog title/close button use the dictionary (were hardcoded Uzbek). Same terminology in uz/ru/en; no visual redesign (existing card/notice styles reused).
- **Creator-experience simplification (2026-07-20, `refactor: simplify creator withdrawal experience`):** the dialog now answers exactly one question — *"How much can I receive today?"* — and shows only four values: **Available to withdraw** (headline), **Withdrawal amount** (entered), **You will receive** (= entered, always), **Remaining available balance** (= available − entered). The **platform-fee row was removed from the dialog entirely** (the fee is already reflected in Available to withdraw, so showing it again invited "will more be taken?" doubt); `previewFee`/`previewGross` were deleted from the component. The amber notice and consent are now **fee-free** (`dash.feeNotice` = "you will receive exactly the amount entered above — with no further deductions"; `dash.feeConsent` = "I confirm this withdrawal request — funds will be sent to my saved card"; `toasts.withdrawAgreeFee` reworded off the word "commission"). The **fee breakdown lives only on the dashboard** now (`CampaignFinancials`), rebuilt as a clean "where the money went" waterfall — **Total donations → Platform fee → Payment provider fee → Completed withdrawals → Pending withdrawals**, then a highlighted **Available to withdraw** total — that reconciles exactly: `totalDonations = platformFee + providerFee + completedWithdrawals + pendingWithdrawals + availableBalance`. Here **Platform fee = total** (collected on paid + reserved on pending + reserved on the available balance), and **Completed/Pending withdrawals are NET** (what the creator received / will receive), so no dashboard figure conflicts with the dialog or history. The **redundant 3-tile funds summary** (Total Raised · Total Withdrawn(gross) · Available) was removed from the `CampaignPayouts` card — the dashboard directly above already ends in Available to withdraw, and its gross "Total Withdrawn" was the one number that conflicted with the new net waterfall; the now-unused `raised`/`totalWithdrawn` props were dropped from `CampaignPayouts`. **No business logic changed** — only which derived values are shown and where; the RPC, ledger, DB CHECK, and `create_payout_request` gross semantics are untouched. Verified: typecheck, 60 tests, production build all green; uz/ru/en updated in parallel.
- **Re-audit + signed waterfall (2026-07-20, `fix: calculate creator available balance using net payout`):** confirmed the *calculation* is NET on every creator surface (`calcAvailableNet` drives the dashboard, dialog headline/Maximum/Max/ceiling; the gross `campaign_available_balance` RPC is on no display path) and made the dashboard breakdown a **signed waterfall** (deductions shown as "−400", muted). ⚠️ **This pass MISDIAGNOSED the user's bug** — see the display-rounding fix below.
- **⭐ ROOT CAUSE of "Available to withdraw shows 10,000" — a DISPLAY-ROUNDING bug, found 2026-07-20 (`fix: display exact so'm on withdrawal/financial surfaces`):** the calculation was correct all along (`calcAvailableNet(10000, []) = 9600`), but `formatMoney` (`lib/utils.ts`) **abbreviates and rounds**: `` `${(amount/1000).toFixed(0)} ming` `` ⇒ `formatMoney(9600) = "10 ming"`, **identical to `formatMoney(10000)`**. So the withdrawal dialog, dashboard headline, Maximum, and history all rendered the correct net 9,600 **as "10 ming"**, indistinguishable from the gross — while the "−400" platform-fee row showed exact (400 < 1,000 is not abbreviated). This is why three prior "still gross" reports could not be reproduced from the source: the net value was right, the *formatter* masked it. A user screenshot of the authenticated page (Total donations "10 ming", Platform fee "−400", Available "10 ming") plus a runtime trace (`node`: raw availableNet = 9600, `formatMoney(9600)`="10 ming", `formatMoneyFull(9600)`="9 600 so'm") pinned it. **Fix:** added `formatAmount(n)` (exact, no abbreviation, no suffix — `9600 → "9 600"`) to `lib/utils.ts` and switched **every money-critical creator surface** to it — `CampaignFinancials` (whole breakdown incl. the highlighted Available) and `CampaignPayouts` (dialog available/amount/you-receive/remaining, min/max hints, request history + details). `formatMoney`'s abbreviation is retained for approximate displays (campaign cards, progress). **No calculation/RPC/ledger/DB change.** Now a 10,000 balance shows **Available to withdraw = "9 600 so'm"**, Max fills 9 600, you-receive = entered, remaining reconciles. Regression tests lock it in (63 total): `formatMoney(9600)==="10 ming"` (documents the hazard), `formatAmount` is exact, and the net at 10k/100k/1M formats to exactly 9 600 / 96 000 / 960 000. Verified: typecheck, 63 tests, production build green. **Lesson:** when auditing a "wrong number" report, verify the *formatter*, not just the computed value.
- **Custom withdrawal-amount validation (2026-07-20, `feat: improve withdrawal amount validation UX`):** replaced the browser's native constraint-validation popup (which appeared in the browser's language and looked unprofessional) with a live, localized inline experience in `CampaignPayouts`. Added `noValidate` to the withdrawal `<form>` so the native tooltip never fires; validation is derived on every keystroke from the same source of truth (`previewNet` vs `[MIN_WITHDRAWAL_NET, availableNet]`). When the entered NET amount is out of range the amount input gets the red-border error state (`border-red-500 focus:ring-red-500`), a red helper message with an `AlertCircle` icon renders directly below it, and **Submit is disabled** (`disabled={submitting || !amountValid}`); when it becomes valid the border/message clear and Submit re-enables — no refresh, no popup. Two localized cases: **exceeds available** ("Withdrawal amount exceeds your available balance." + "Maximum available today: {max}") and **below minimum** ("Minimum withdrawal is {min}."), with `{max}`/`{min}` interpolated as exact `formatAmount` so'm. Keys `dash.withdrawErrExceeds`/`withdrawErrMaxToday`/`withdrawErrBelowMin` added in uz/ru/en. Accessibility: input carries `aria-invalid` + `aria-describedby` (pointing to the error when present, the min/max hint otherwise), the message is `role="alert"` (screen-reader announced), and the label is tied via `htmlFor`/`id`. No redesign — reuses the existing `.input`, red-500 helper-text and icon idioms; the submit-path guards remain as a server-side-echoing fallback. Verified: typecheck, 63 tests, production build green.
- **Withdrawal preview gated on validity (2026-07-20, `feat: improve withdrawal preview validation experience`):** the financial preview (Withdrawal amount / You will receive / Remaining available balance) previously rendered whenever `previewNet > 0`, so an out-of-range entry (e.g. 10,000 against a 9,600 balance) showed an **impossible transaction**. It now renders **only when `amountValid`** (the same gate that drives the red error + disabled Submit). For an empty or invalid amount it is replaced by a calm informational placeholder (`Info` icon + "Preview unavailable" + "Enter a valid withdrawal amount between {min} and {max} to preview your payout.") styled with the same `bg-gray-50` card idiom — not an error/alert look — so it reads as "calculations will appear once the amount is valid". `aria-live="polite"` announces the swap; it flips instantly on each keystroke with no refresh. Keys `dash.previewUnavailableTitle`/`previewUnavailableBody` added in uz/ru/en (`{min}`/`{max}` = exact `formatAmount` so'm). No redesign. Verified: typecheck, 63 tests, production build green.
- **Preview-placeholder polish + Use-maximum shortcut (2026-07-20, `feat: polish withdrawal validation experience`):** the invalid-state placeholder was upgraded from a bare "Preview unavailable" line to a helpful **"Payout preview"** card — "Enter an amount between {min} and {max} to see:" followed by two bullets ("Amount you'll receive", "Remaining balance after withdrawal") — so it previews *what* will appear, using the same calm `bg-gray-50` card (no warning/error styling). Keys renamed `previewUnavailableTitle`→`previewTitle`, `previewUnavailableBody`→`previewIntro`, plus `previewBullet1`/`previewBullet2`. When the entered amount **exceeds the maximum**, a secondary text button **"Use maximum amount"** (`dash.useMaxAmount`) appears under the "Maximum available today: {max}" line; clicking it fills the exact `availableNet`, which is valid, so the payout preview appears immediately. All keys in uz/ru/en. No redesign. Verified: typecheck, 63 tests, production build green.
- **Dynamic-values consistency pass (2026-07-20, `feat: improve withdrawal validation and dynamic payout preview`):** confirmed every value on the validation/preview path is derived at runtime — `MIN_WITHDRAWAL_NET` (business-rule constant, single source of truth) and `availableNet` (= `calcAvailableNet(current_amount, payoutRequests)`, which recomputes as donations complete / withdrawals become paid or pending) — with **no hardcoded monetary values anywhere** in the dialog. Simplified `amountError` (object) → `amountErrorMsg` (string) and made the **"Maximum available today: {max}" line + "Use maximum amount" button show for ANY invalid entry** (below-minimum as well as over-maximum), not only over-max — so the max reference and one-tap fill are always offered when the amount is invalid. The exceeds message no longer embeds the max (it now renders once, below the message, for all cases). No new strings (reuses `withdrawErrMaxToday`/`useMaxAmount`/`withdrawErrExceeds`/`withdrawErrBelowMin`). Verified: typecheck, 63 tests, production build green.
- **Admin-transfer net-amount fix + single source of truth (2026-07-20, `fix: admin withdrawal review shows net transfer amount`):** the **Admin Withdrawal Review** page displayed amounts with `formatMoney` — which rounds 9,600 → "10 ming", identical to the gross "10 ming" — and never prominently labelled the transfer amount, so an admin could transfer the **gross** (10,000) while XAYR still kept the 4% fee → permanent over-payment. Fix (app, deployed): (1) new **`payoutBreakdown(row)`** in `lib/payout.ts` — the ONE reader of a stored request's `{gross, fee, net, ratePercent}`; reads stored `amount`/`commission_amount`/`payout_amount` (never re-derives — historical 0%/3%/4% rows keep their rate; DB CHECK guarantees fee+net=gross). Used by the admin page AND the creator history/details, so no surface can disagree. (2) `AdminPayouts` switched `formatMoney`→`formatAmount` (exact) everywhere, and the review modal now shows **Requested → Platform fee (n%) → a prominent brand-highlighted "Admin o'tkazishi kerak" = NET** with "transfer exactly this; the fee stays with XAYR" — the list row leads with the net too. (3) Verified the RPCs (`create_payout_request` computes the fee once; `approve`/`reject`/`mark_payout_paid` only transition status) and the ledger (`withdrawal = −payout_amount`, `platform_fee = −commission`, both stored) — **no fee is computed twice anywhere** (grep: `*0.04` exists only in `lib/payout.ts` + the SQL RPC). (4) Migration **#63 `notify-payout-net.sql`** words creator notifications with the net (was gross). 6 new tests (69 total): 0%/3%/4%/large/min/rounding breakdowns, reconciliation, and net≠gross formatting. Emails/receipts: none exist yet (no transactional-email provider), so nothing to change there. Verified: typecheck, 69 tests, production build green.
- **Withdrawal-workflow audit + funded-campaign fix (2026-07-20, `fix: complete manual withdrawal request workflow`):** full trace of the request path (client `submit` → `create_payout_request` RPC → `payout_requests` insert + event → admin dashboard → `mark_payout_paid`/`reject` → ledger triggers → creator notification). **Root cause of "fails with a generic error": a status-guard mismatch.** The UI offers Withdraw on **`funded`** campaigns (`MyCampaigns.tsx:324` = `['active','funded']`; the withdraw page's `approved` gate = active/completed/funded), but `create_payout_request` only accepted `('active','completed')` and raised `campaign_not_approved` for funded — the `funded` status arrived later (#41) and the payout RPC (#40) was never widened. So a creator whose campaign reached its goal (→ funded via `close_campaign`/`expire_due_campaigns`) submits a valid request and it is rejected despite the funds being present. **Fix:** migration **#61 (`payout-allow-funded.sql`)** widens the guard to `('active','completed','funded')` (only that line changes vs #60) — **must be run in the Supabase SQL Editor**; cannot be applied from the app deploy. **Error handling hardened (deployed):** `CampaignPayouts.submit` now (a) never hides the error — an unmapped/DB fault is `console.error`-logged in full (SQLSTATE + message + details + hint + campaignId/gross), (b) maps every RPC `raise` code to a localized message (added `campaign_not_found`, `insufficient_balance`, `below_minimum`), and (c) shows a friendly localized fallback `toasts.withdrawFailed` ("Could not submit your withdrawal request…") instead of the vague `toasts.generic`, plus a `try/catch` around the RPC for network faults. New toast keys in uz/ru/en. The RPC call itself was verified correct (3-arg `(uuid,integer,text)`, matches `types/index.ts` and the deployed #40/#51/#60 signature; sends the exact gross via `grossForNet`). Balance/reserve/ledger/notification wiring were audited and found correct (available = `calcAvailableNet`; funds reserved by `campaign_available_balance` counting active+paid; `paid`→ledger `withdrawal`+`platform_fee` via #45 triggers; `notify_on_payout_event`). Verified: typecheck, 63 tests, production build green.
- **Withdrawal error root cause (earlier):** *"Could not find the 'card_number' column of 'payout_accounts' in the schema cache"* is a live-DB issue — the repair migration **#59 (`payout-accounts-schema-repair.sql`) must be run in the Supabase SQL Editor**; it cannot be applied from the app deploy.
- **Net-request model — SHIPPED 2026-07-20 without a migration.** The 2026-07-19 recommendation proposed a DB migration (store `payout = net`, `commission = round(net/24)`) applied atomically with a client change, and was deferred over the deploy-order hazard. The shipped approach sidesteps the hazard entirely: the client converts the entered net to the **exact** gross the unchanged RPC expects (`grossForNet` inverts the server's own rounding formula, proven exhaustively in tests), so there is no window in which client and server semantics can disagree — old clients keep working, the DB, ledger, admin views and historical rows are untouched, and the creator gets the full net UX (enters 9,600, receives 9,600, remaining 0).
...
- **Mobile UX audit (2026-07-20):** audited the app at 320–768px against the live site with scripted checks (overflow, tap-target sizes, safe-area). The base was already strong — **no horizontal scroll at 320px**, 16px form inputs (no iOS focus-zoom), safe-area padding on the bottom nav, an in-flow spacer preventing CLS/overlap, the campaign-detail donate bar offset to `bottom-[calc(4rem+env(safe-area-inset-bottom))]` (clears the tab bar), reduced-motion support, and comfortable 46–48px auth buttons. Fixes applied and **verified on production** (both gated so **desktop is visually identical** — confirmed hero 60px@768 / 72px@desktop): (1) **hero `<h1>`** was a fixed `text-5xl` (48px) that overflowed long Uzbek words (e.g. "osonlashtiramiz") on 320–360px → now fluid `text-[clamp(2rem,8.5vw,3rem)]` (verified 32px@320, fits the viewport), `sm:`/`lg:` unchanged; (2) **toasts** (bottom-right) now clear the mobile bottom nav via a `<lg` CSS offset so they never sit on the tab bar; desktop unchanged. No redesign; branding/colors unchanged. **Not changed:** enlarging the secondary overlay tap targets (SaveButton 36px, footer social icons 32px) to 44px was attempted but the `sm:`/`max-sm:` width variants did not gate to mobile in this build (they leaked the larger size to desktop, unlike the text variants which gate correctly), so those were reverted to keep desktop provably identical — a follow-up can enlarge them via a hit-area wrapper.
- **Status:** ✅ Complete.
...
| 61 | `payout-allow-funded.sql` | **Allow withdrawals from FUNDED campaigns (workflow bugfix)** — recreates `create_payout_request` with the status guard widened `('active','completed')` → `('active','completed','funded')`. The withdrawal UI already offers Withdraw on `funded` (goal-reached) campaigns, but the RPC (from #40, before `funded` existed in #41) rejected them with `campaign_not_approved`. Everything else is identical to #60 (3-arg sig, 4% commission, `v_min=5000`, all guards/snapshots/events). Function body only. | **Required so goal-reached (funded) campaigns can withdraw** | 40, 51, 60 |
| 62 | `payout-write-path-repair.sql` | **Withdrawal write-path repair (debugging)** — idempotently re-asserts the whole `create_payout_request` path (payout_requests #26/#40 columns + sum CHECK, `payout_accounts.card_number`, financial_ledger #46 columns + widened entry_type CHECK, the lifecycle trigger, the 3-arg funded-allowed RPC) + `notify pgrst,'reload schema'`. Fixes schema/cache drift (PGRST202 / 23514 / 42703). No data touched. Pair with `diagnose-withdrawal.sql`. | Apply only if a withdrawal fails with a raw DB error | 40, 45, 46, 51, 60, 61 |
| 63 | `notify-payout-net.sql` | **Payout notifications show the NET (financial consistency)** — recreates `notify_on_payout_event` so messages carry the net received (created shows requested+fee+net; approved/paid show net) instead of only the gross. Reads stored values, never re-derives. Function body only; reuses the #19 trigger. | Recommended so notifications match the amount actually paid | 19, 26, 40 |
| 64 | `saved-payment-methods.sql` | **Saved cards (Click card tokens)** — new `saved_payment_methods` table (encrypted token + last4/brand/holder, default/active, audit ts) + partial-unique one-default index + RLS (owner select on display cols; `token_ciphertext`/`enc_version` column-REVOKEd from anon/authenticated → service-role only) + SECURITY DEFINER RPCs `save_card`/`set_default_card`/`deactivate_card`/`delete_card`. Additive only — `donations`/ledger/callbacks untouched. | **Required only if enabling saved cards** | 1 |
...
**Embedded card payment (checkout.js) — official widget, opt-in (restored 2026-07-23).** Opt-in in-page card window (UZCARD/HUMO) via Click's **official** `checkout.js` (`window.createPaymentRequest`, opened exactly as documented), so the donor stays on xayr.uz and types the card number/expiry in **Click's own** widget. **Default OFF** — every donor keeps the proven redirect until `NEXT_PUBLIC_CLICK_EMBEDDED_CARD=1` is set (unset to roll back instantly; no migration, no data change). If checkout.js is unavailable the client falls back to the redirect.
- **On the "my.click.uz does not allow the connection" incident (2026-07-23):** the frame that error came from is **Click's own** widget (`createPaymentRequest` embeds Click's card form from `my.click.uz`) — **not** a custom iframe/modal in our code (the codebase has **zero** `<iframe>` and never frames a `my.click.uz`/redirect URL; CSP already allows `my.click.uz`). The error appears when `my.click.uz` refuses to be framed — i.e. the Click merchant cabinet isn't configured to allow the pay-by-card widget on the site's origin (Click is still unverified against a live/sandbox merchant). The correct fix is **not** to drop the official widget (a prior over-broad rollback did that): keep it **opt-in** so donors get the safe redirect by default, and only set `NEXT_PUBLIC_CLICK_EMBEDDED_CARD=1` once Click confirms the widget is enabled for the domain. Our integration is iframe-free and matches Click's docs. Card data goes **straight to Click** (*"Данные карты не передаются мерчанту"*), so Xayr never sees/stores a PAN and stays **out of PCI scope**. `createPayment` returns an `embedded` payload **alongside** `redirectUrl`, so a blocked/offline library, or CSP failure, silently falls back to the redirect rather than stranding the donor. The client-side `status` is **UX only and never credits** — `confirmDonation()` from a verified server callback remains the sole crediting path. CSP grants `my.click.uz` (script/frame/connect). `lib/payments/click-checkout.ts`, `docs/click-embedded-card.md`.
...
**Saved Cards — AUTOMATIC token saving (redesigned 2026-07-23).** OPTIONAL, additive: an authenticated donor can pay with a previously saved card. There is **no manual "add / register a card" flow** — cards are saved **automatically after a successful donation**, and only when Click actually returns a reusable token.
- **New donation flow.** The donor picks **Bank card** and uses the **normal Click payment flow** (Checkout JS / redirect) — unchanged, no extra step, no card-saving prompt. On the SHOP-API **Complete** callback, after the single `confirmDonation()` credits the donation, an **automatic** best-effort step runs: if Click's callback carried a **reusable `card_token`**, it is encrypted (AES-256-GCM) and stored in `saved_payment_methods`, associated with the donor, `is_active=true` (first card becomes default). If Click returns **no** token, the donation completes normally and **nothing is saved** — no error, no UI. Flow: **confirm donation → save token (if available) → finish.**
- **Never blocks a donation.** `saveDonationCardToken()` (`lib/payments/save-card-token.ts`) is fully guarded — it **never throws and never blocks**; any failure is logged and swallowed. It skips silently for guest donations (no user), when the cipher is unconfigured, or when the same card (brand+last4) is already saved. The token is read from the **raw** callback form, so it never lands in the logged `payment_events` params. The service-role callback **inserts directly** into `saved_payment_methods` (bypassing RLS/grants) because the `save_card` RPC needs the caller's `auth.uid()`; the existing schema + `token-cipher` are reused (no new table, **no migration**, no duplicated payment logic).
- **Future donations.** If the donor has saved cards, the donation form shows **`SavedCardSelector`**: their **saved cards** first, then **"Use another card"** → the normal Click card payment flow. Selecting a saved card charges it via the unchanged `card-token/pay` → `confirmDonation()` path. **Account → Saved Cards** (`/profile/cards`) lets them **view / set-default / delete** — no add action.
- **Removed (2026-07-23):** the whole registration path — `AddCardFlow` (card inputs, SMS-OTP request, OTP button), the enrolment routes **`/api/account/cards/{request,verify}`**, the `CHOICE_ADD` chooser option, and the `NEXT_PUBLIC_CLICK_CARD_REGISTRATION` flag. **Kept:** `saved_payment_methods` schema + RPCs, `lib/crypto/token-cipher.ts`, `lib/payments/providers/click-card-token.ts`, `card-token/pay`, the list/delete/set-default routes, and the encryption tests.
- **Reality check (unchanged):** Click's Checkout JS / SHOP callback does **not currently emit a reusable token**, so in practice the automatic save is **inert until Click provides one** — exactly the "no token ⇒ don't save, no error" branch. The seam is in place: the moment a `card_token` arrives on the Complete callback, cards begin saving automatically with no further code change. Still gated by `NEXT_PUBLIC_CLICK_SAVED_CARDS` (whole feature) + `PAYMENT_TOKEN_ENC_KEY`; off ⇒ the donation form is byte-for-byte today's Checkout JS and `/profile/cards` 404s.
- **Where:** `lib/payments/save-card-token.ts` (auto-save + `clickReusableToken`), `app/api/payments/click/route.ts` (calls it after confirm), `supabase/saved-payment-methods.sql` (#64), `lib/crypto/token-cipher.ts`, `lib/payments/providers/click-card-token.ts`, `app/api/account/cards/{route,[id]}` + `app/api/payments/click/card-token/pay/route.ts`, `components/payments/{SavedCardSelector,saved-card-constants}.ts(x)` + `components/profile/SavedCards.tsx` + `app/[locale]/profile/cards/page.tsx`, `components/donations/DonationForm.tsx`, `locales/*` `cards.*`. Encryption unit-tested (65 tests).
...
- **Coverage:** `locales/{uz,ru,en}/common.json` — **exact key parity: 1366 leaf keys each** (verified programmatically, zero missing/extra). Server dictionaries loaded lazily (`i18n/dictionaries.ts`); client via `useI18n()` context (instant switch, no reload).
- **Withdrawal flow + persistence pass (2026-07-19):** the remaining non-toast Uzbek labels in the withdrawal flow (`CampaignPayouts` — event-log action labels, blocked-reason messages, request-detail modal labels [commission / payment details / note / admin note / reference / history], the notes field label + placeholder, and the Cancel / Submit / Close buttons) are now `t()`-driven (18 new `dash.*` keys; generic buttons reuse `ux.cancel/submit/close`, which already carried the same Uzbek wording), so a Russian/English creator no longer sees mixed Uzbek on that screen. **Language persistence verified** for before/after login, logout, registration, password reset, Google OAuth, page refresh, browser tabs, the donation/withdrawal flows, and payment-provider returns: locale lives in every content URL; the `NEXT_LOCALE` cookie (set by `LanguageSwitcher`, `SameSite=Lax`, 1 yr) drives the middleware redirect for locale-less landings (`/auth/callback` → `/`, `/payment/success`, `/auth/reset-password`); signed-in users persist `users.preferred_language` (which push notifications use for the click-through locale).
...
- **Remaining work (known):** the withdrawal flow (`CampaignPayouts`) is now fully localized (2026-07-19), but a full app-wide sweep is **not** complete. Still Uzbek-only: (1) **Admin panel** components (`AdminPayouts`, `AdminDonationsReconciliation`, `AdminPaymentProviders`, `AdminVerifications`, `AdminUsersManager`) — internal-operator surface, deliberately deferred. (2) **API route error prose** (e.g. login/signup) is still returned in Uzbek server-side; the client shows it as a fallback after its own translated message. Localizing it properly means returning stable error codes + client mapping — deferred to avoid changing auth/donation response contracts. (3) Several campaign/donation/profile components (`CompletionReportForm`, `CreateCampaignForm`, `DonationForm`, `ProfileForm`, `CampaignUpdates`) still have hardcoded non-toast labels. (4) Add a CI check that locale files stay key-aligned. These are the concrete follow-ups for full "no hardcoded string" coverage.
