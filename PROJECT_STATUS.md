...
> **Branch:** main · **Latest at sync:** admin withdrawal review now shows the NET transfer amount prominently (was rounded gross "10 ming") via a single `payoutBreakdown()` source of truth + net-amount notifications (#63) — on top of the withdrawal-workflow audit (funded-campaign RPC #61) + validation/preview polish + display-rounding fix (`formatMoney` rounded net 9,600 → "10 ming"; now exact "9 600 so'm") + simplified withdrawal UX + net-display payout model (no migration) + Payme (#48 pending) + Click
>
...
| Testing / CI | 55% | CI runs **lint (ESLint next/core-web-vitals) + typecheck + tests + build** on `npm ci` + committed lockfile. Focused **Vitest** suite (60 tests) covers Click/Payme callback verification, amount/currency validation, commission math + the net↔gross withdrawal conversion + available-balance/net-display scenarios (10k/100k/1M, multiple/pending withdrawals, refunds), and `confirmDonation` idempotency; no RLS/integration tests yet. |
