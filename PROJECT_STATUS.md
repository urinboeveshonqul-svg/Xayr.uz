...
- **Verified against official Click docs** ([docs.click.uz](https://docs.click.uz/en/merchant-api-request/)): checkout.js issues **no** token; the **Card Token API** (`/v2/merchant/card_token/{request,verify,payment,delete}`) requires the Merchant-API secret `Auth` header (`merchant_user_id:sha1(ts+secret):ts`) on every call → **browser-direct tokenization is impossible** (secret can't be exposed), so the PAN transits the backend for `request` only → **PCI scope (SAQ A-EP)** for this path (Checkout JS stays out of scope). `card_token/payment` is **synchronous** (returns `payment_id`/`payment_status`, no Prepare/Complete) → finalised **response-driven** through the **unchanged `confirmDonation()`** (still the sole crediting authority).
- **Flow:** enrol on the donation form ("Add a new card and save it") or in **Account → Saved Cards** — `request` (PAN+expiry+phone → Click SMS-OTP) → `verify` (OTP) → token **AES-256-GCM-encrypted** and stored; the unverified token rides to the browser only inside a server-encrypted envelope, never in clear. Charge = create the normal `pending` donation (`/api/donations`, method `click`) → `card-token/pay` (server decrypts, `card_token/payment`) → `confirmDonation()`. Failure → retry / another card / Checkout JS (never blocks); dead token → auto-deactivated.
...
- **⛔ Before enabling — unresolved with Click (do NOT flip on until confirmed):** **Q7** persistent-token TTL/expiry · **Q8** exact invalid/expired-token `error_code` values (`CLICK_INVALID_TOKEN_ERROR_CODES` is intentionally EMPTY until known) · the `card_token/payment` amount unit + `payment_status` success value · **Q9** Card Token API enabled on the merchant account. Like Click/Payme, this is **code-complete but operationally blocked** pending merchant verification.
