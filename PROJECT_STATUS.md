...
- **Where:** `app/[locale]/layout.tsx` + `components/layout/SiteChrome.tsx`, `app/[locale]/campaigns/[slug]/page.tsx`, `components/campaigns/CampaignDetail.tsx`; `app/[locale]/campaigns/page.tsx`, `app/[locale]/page.tsx`, `supabase/optimize-campaign-indexes.sql`, `next.config.mjs`.
- **Status:** ✅ Solid for current scale; navigation smoothness improved (Navbar persists, fewer per-nav round-trips). Deferred to a separate task (per the perf audit): middleware `getUser()` on every navigation, and the full translation dictionary shipped to the client.
