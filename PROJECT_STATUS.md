...
- **Extension (Phase 2):** owner of an **expired, under-goal** campaign can request a new deadline (KYC required, ≤30 days out, ≤2 extensions, one pending at a time) from the dashboard. Admin reviews under **/admin/extensions** → Approve reactivates the campaign (`active` + new deadline, restoring discovery + donations) / Reject keeps it expired; owner is notified either way. All writes go through SECURITY DEFINER RPCs (`request_/approve_/reject_campaign_extension`); clients only read their own requests.
- **Where:** `supabase/campaign-expiration.sql` + `supabase/campaign-extensions.sql`; `app/api/cron/expire-campaigns/route.ts` + `vercel.json` cron (daily); `lib/utils.ts` (`isCampaignEnded`/`isGoalReached`/`STATUS_CONFIG`); `components/campaigns/CampaignDetail.tsx`; `app/api/donations/route.ts`; `components/profile/MyCampaigns.tsx` (history filters + extend modal); `components/admin/{AdminCampaignsManager,AdminExtensions}.tsx` + `app/[locale]/admin/extensions/page.tsx`.
