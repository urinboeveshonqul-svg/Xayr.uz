...
> **Branch:** main · **Latest at sync:** simplified creator withdrawal UX (dialog shows only Available/Amount/You-receive/Remaining; fee breakdown moved to the dashboard waterfall) on top of the net-display payout model (no migration) + Payme (Merchant API, #48 pending) + provider selection (#47 ✅ applied) + Click (SHOP API)
>
...
- **Re-audit + signed waterfall (2026-07-20, `fix: calculate creator available balance using net payout`):** a full re-audit (prompted by a repeat "dashboard shows gross 10,000" report) **confirmed the calculation is already NET on every creator surface** and found **no gross display path**: `calcAvailableNet` drives the dashboard headline, the dialog headline/Maximum/Max/amount ceiling; the gross `campaign_available_balance` RPC is called from **no** `.tsx` display path (only comments/types reference it); `MyCampaigns` shows "Total raised" (legitimately gross) not an available figure; the analytics page only links to `/withdraw`; `lib/finance` is admin/transparency-only. **Production was verified live** serving the current commit (HTML `sentry-release` = deployed HEAD, `sentry-environment=production`, `x-vercel-cache: MISS`, `cf-cache-status: DYNAMIC` ⇒ freshly server-rendered, not edge/browser-cached; no service worker). So a 10,000 balance already renders **9,600** as Available to withdraw in production. The only change this pass is **presentational**, matching the requested spec exactly: the dashboard breakdown now renders the four deductions as **signed negatives** ("−400 so'm", muted) so the list reads as `Total donations − platform fee − provider fee − completed − pending = Available to withdraw`, making it unmistakable that the highlighted result (net) is what's left after deductions — and distinguishing it from the "Total donations" row (10,000) directly above. **No calculation, RPC, ledger, or DB change.** A residual "10,000 as available" seen by a user is a stale client view (hard-refresh needed) or the Total-donations row being misread — not the deployed code. Verified: typecheck, 60 tests, production build green.
