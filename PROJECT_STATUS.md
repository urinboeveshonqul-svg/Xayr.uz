...
**Saved Cards (Click Card Token API) — code-complete, OFF by default (2026-07-22).** OPTIONAL, additive enhancement letting an authenticated donor pay with a previously saved card. **Checkout JS is unchanged and remains the default / "Use another card".**
- **⛔ REGISTRATION TEMPORARILY DISABLED (2026-07-23).** Enrolling a **new** card — Click's Card Token **SMS-OTP** flow (`card_token/request` + `verify`) — is turned off because that flow is not working reliably; exposing a broken "Add and save a new card" is worse than hiding it. **Only enrolment is gated** — everything else is unchanged: **existing saved cards can still be selected and charged**, one-time **Checkout JS** payments, **donations**, and **payment confirmation** all work as before. What's hidden/blocked: the donation-form **"Add and save a new card"** option (`SavedCardSelector` `allowAdd`), the **Account → Saved Cards** *Add* button + `AddCardFlow` (SMS request, card inputs, OTP button), and the enrolment routes **`/api/account/cards/{request,verify}` → 503**. No error surfaces because the controls are removed, not left dead. The implementation, DB schema, API routes, helpers and encryption tests are all **preserved** behind the flag. **To re-enable** once the Click flow is fixed: set **`NEXT_PUBLIC_CLICK_CARD_REGISTRATION=1`** (single flag; helper `isCardRegistrationEnabled()` in `components/payments/saved-card-constants.ts`). This is independent of `NEXT_PUBLIC_CLICK_SAVED_CARDS` — the whole saved-cards feature must be on for either to appear.
- **Verified against official Click docs** ([docs.click.uz](https://docs.click.uz/en/merchant-api-request/)): checkout.js issues **no** token; the **Card Token API** (`/v2/merchant/card_token/{request,verify,payment,delete}`) requires the Merchant-API secret `Auth` header (`merchant_user_id:sha1(ts+secret):ts`) on every call → **browser-direct tokenization is impossible** (secret can't be exposed), so the PAN transits the backend for `request` only → **PCI scope (SAQ A-EP)** for this path (Checkout JS stays out of scope).
- **Confirmed by Click support (2026-07-22) — drives the finalization design:** `card_token/payment` `amount` is **UZS (so'm)**, not tiyin; `payment_status` = **0 error / 1 processing / 2 success**; `temporary=0` = **persistent token, no expiry**; `card_token/delete` **permanently** invalidates the token (must re-enrol); Card Token API is **enabled on the merchant account**; and — critically — **after a successful `card_token/payment`, Click ALSO sends the normal SHOP API Prepare + Complete callbacks.** So the saved-card path does **NOT** finalise from the payment response. It only **places the charge** (accepts `payment_status` 1/2) and leaves the donation `pending`; the **existing Complete callback (`app/api/payments/click`) is the SINGLE finalizer via the unchanged `confirmDonation()` for BOTH Checkout JS and Saved Cards.** This removes the earlier direct-confirm call (which would have completed the donation before Prepare arrived → Prepare answering `AlreadyPaid` −4, a race). No duplicate finalization path remains.
- **Flow:** enrol on the donation form ("Add a new card and save it") or in **Account → Saved Cards** — `request` (PAN+expiry+phone → Click SMS-OTP) → `verify` (OTP) → token **AES-256-GCM-encrypted** and stored; the unverified token rides to the browser only inside a server-encrypted envelope, never in clear. Charge = create the normal `pending` donation (`/api/donations`, method `click`) → `card-token/pay` (server decrypts, calls `card_token/payment`, returns `accepted`) → client redirects to `/payment/success` which **polls** → Click **Prepare → Complete → `confirmDonation()`** credits (identical to Checkout JS). Failure → retry / another card / Checkout JS (never blocks); dead token → auto-deactivated.
- **Where:** `supabase/saved-payment-methods.sql` (#64), `lib/crypto/token-cipher.ts`, `lib/payments/providers/click-card-token.ts`, `app/api/account/cards/**` + `app/api/payments/click/card-token/pay/route.ts`, `components/payments/{AddCardFlow,SavedCardSelector}.tsx` + `components/profile/SavedCards.tsx` + `app/[locale]/profile/cards/page.tsx`, `components/donations/DonationForm.tsx` (conditional block only), `locales/*` `cards.*`. Gated by `PAYMENT_TOKEN_ENC_KEY` + `CLICK_MERCHANT_USER_ID` (server) and `NEXT_PUBLIC_CLICK_SAVED_CARDS` (whole feature). **One flag fully disables it:** off ⇒ the donation form is byte-for-byte today's (no saved-card branch renders, no `/api/account/cards` fetch, no guest impact), `/profile/cards` returns **404** (`notFound()`), and `SavedCardSelector`/`AddCardFlow` are **lazy-loaded** (`next/dynamic`) so their code is a separate chunk never shipped in the Checkout JS bundle when off. Server routes independently 503 without the secret env. Encryption unit-tested (65 tests).
- **⛔ Before enabling — remaining unknown:** only **Q8** — the exact invalid/expired-token `error_code` value (Click confirmed `payment_status` but not this), so `CLICK_INVALID_TOKEN_ERROR_CODES` stays **EMPTY** until known (auto-deactivation inert; a dead token surfaces as a generic decline). Q7 (no expiry), Q9 (API enabled), the amount unit (UZS) and `payment_status` (0/1/2) are now **confirmed**. Still needs a **sandbox charge** to confirm Prepare/Complete arrive for a `card_token` payment and the poll reaches `completed`. Code-complete, OFF by default.
