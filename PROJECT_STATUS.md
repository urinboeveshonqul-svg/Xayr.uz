...
**Embedded card payment (checkout.js) — ⛔ DISABLED (production regression, 2026-07-23).** The in-page card window opens Click's own overlay pointing at `my.click.uz`, which **refuses to be framed** ("the site my.click.uz does not allow the connection") — so enabling it broke card payments on desktop **and** mobile. `isClickEmbeddedCardEnabled()` is now **forced `false`** (regardless of the env flag) and the client opener was removed from `DonationForm`, so **every donor uses the proven full-page redirect** to Click's hosted checkout. Confirmation/callbacks/ledger/balances/saved-cards are all unchanged. Re-enable only after re-validating the in-page flow against a live merchant: restore the env read in `isClickEmbeddedCardEnabled` AND the client opener. Original opt-in design (kept for reference below):
- Opt-in in-page card window (UZCARD/HUMO) via Click's official `checkout.js`, so the donor never leaves the campaign page. **Default OFF** — every donor keeps the proven redirect until `NEXT_PUBLIC_CLICK_EMBEDDED_CARD=1` is set (unset it to roll back instantly; no migration, no data change). Card data goes **straight to Click** (*"Данные карты не передаются мерчанту"*), so Xayr never sees/stores a PAN and stays **out of PCI scope**. `createPayment` returns an `embedded` payload **alongside** `redirectUrl`, so a blocked/offline library, or CSP failure, silently falls back to the redirect rather than stranding the donor. The client-side `status` is **UX only and never credits** — `confirmDonation()` from a verified server callback remains the sole crediting path. CSP grants `my.click.uz` (script/frame/connect). `lib/payments/click-checkout.ts`, `docs/click-embedded-card.md`.
