...
- **Deferred:** deleting a draft (or removing a draft image mid-edit / abandoning a draft) does **not** garbage-collect the already-uploaded objects in the `campaign-images` bucket — storage cleanup is deferred (a future scheduled sweep of unreferenced own-folder objects). No correctness/privacy impact; the bucket is public-read own-folder and the URLs are unguessable.
