...
> **Branch:** main · **Latest at sync:** withdrawal-workflow audit — funded-campaign RPC fix (migration #61, manual apply) + hardened error handling (real error logged, localized friendly messages, no generic "something went wrong") on top of the withdrawal validation/preview polish + display-rounding fix (`formatMoney` rounded net 9,600 → "10 ming"; now exact "9 600 so'm") + simplified withdrawal UX + net-display payout model (no migration) + Payme (#48 pending) + Click
>
