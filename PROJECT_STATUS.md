...
> **Branch:** main · **Latest at sync:** polished withdrawal validation (helpful "Payout preview" placeholder + "Use maximum amount" shortcut when over the max) on top of the valid-amount preview gating + custom localized amount validation (inline red error + disabled Submit, no native browser popup) + the display-rounding fix (`formatMoney` rounded net 9,600 → "10 ming"; now exact "9 600 so'm") + simplified withdrawal UX + net-display payout model (no migration) + Payme (#48 pending) + Click
>
...
- **Withdrawal error root cause:** *"Could not find the 'card_number' column of 'payout_accounts' in the schema cache"* is a live-DB issue — the repair migration **#59 (`payout-accounts-schema-repair.sql`) must be run in the Supabase SQL Editor**; it cannot be applied from the app deploy.
