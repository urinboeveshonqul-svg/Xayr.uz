...
> **Branch:** main · **Latest at sync:** withdrawal preview gated on a valid amount (informational placeholder instead of an impossible-transaction summary) + custom localized amount validation (inline red error + disabled Submit, no native browser popup) on top of the display-rounding fix (`formatMoney` rounded net 9,600 → "10 ming"; now exact "9 600 so'm") + simplified withdrawal UX + net-display payout model (no migration) + Payme (#48 pending) + Click
>
