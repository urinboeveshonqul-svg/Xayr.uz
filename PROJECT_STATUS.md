...
> **Branch:** main · **Latest at sync:** net-display payout model + withdrawal-dialog transparency pass (creator sees/enters/receives the same NET amount; no migration) on top of Payme (Merchant API, #48 pending) + provider selection (#47 ✅ applied) + Click (SHOP API)
>
...
- **Withdrawal-dialog transparency pass (2026-07-20, same-day follow-up):** the dialog now leads with a prominent **Available to withdraw** block (brand-tinted card matching the dashboard tile: label + amount + one-line explanation of where the number comes from — completed donations minus platform fee and previous/pending withdrawals, `dash.availableExplain`); the amber notice reads "the available amount already includes the platform fee deduction — you will receive exactly the amount entered above" (`dash.feeNotice`); the summary labels are "Withdrawal amount / Platform fee (4%, already deducted) / You will receive / Remaining available balance"; a short **after-submit note** (`dash.afterSubmitNote`: reviewed in 1–3 business days → sent to the card above → notification) states the process at the decision point, since the page's fuller processing card is hidden behind the modal overlay; and the dialog title/close button use the dictionary (were hardcoded Uzbek). Same terminology in uz/ru/en; no visual redesign (existing card/notice styles reused).
