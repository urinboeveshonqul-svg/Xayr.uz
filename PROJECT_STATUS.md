...
> **Branch:** main · **Latest at sync:** net-display payout model (creator sees/enters/receives the same NET amount; no migration) on top of Payme (Merchant API, #48 pending) + provider selection (#47 ✅ applied) + Click (SHOP API)
>
...
| Testing / CI | 55% | CI runs **lint (ESLint next/core-web-vitals) + typecheck + tests + build** on `npm ci` + committed lockfile. Focused **Vitest** suite (51 tests) covers Click/Payme callback verification, amount/currency validation, commission math + the net↔gross withdrawal conversion + available-balance scenarios, and `confirmDonation` idempotency; no RLS/integration tests yet. |
