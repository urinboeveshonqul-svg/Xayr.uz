...
- **Financial rule (audited 2026-07-19):** the 4% platform fee is **deducted FROM the withdrawal amount** — the creator enters a gross amount (5,000..available), the GROSS leaves the balance, and they receive `amount − fee` (`payout_amount`). Enforced by the DB `CHECK (commission_amount + payout_amount = amount)`. A full audit confirmed every surface already uses this rule consistently: the dialog + history + admin show "you receive" = `payout_amount` (net), the ledger records `withdrawal = −payout_amount` and `platform_fee = −commission`, `financial_summary` splits gross/net/fee, and `campaign_available_balance` deducts the gross. The withdrawal dialog now also shows a live **Remaining balance** = `available − amount` (0 when withdrawing the full balance), matching the balance the gross deduction actually leaves. No calculation was changed.
- **Financial-transparency audit (2026-07-20) — canonical answers:** (1) **When platform fees are deducted:** the 4% is computed and stored at request time (`commission_amount = round(amount × 0.04)`), taken FROM the requested gross; ledgered as `platform_fee` when the payout is marked **paid**. Donors are never charged. (2) **When provider fees are deducted:** **never, currently** — `providerFee = 0` on every surface; the `provider_fee` ledger type is reserved but always 0 (no gateway charges one in the live flow yet). (3) **What "Available to withdraw" represents:** `campaign_available_balance` = completed donations (`current_amount`) − Σ gross of active+paid payout requests = the max **gross** the creator can request today (withdrawing it yields `available − 4%` net). **One calculation source:** the withdraw page derives all figures from these ledger-backed values and passes them to both the dashboard (`CampaignFinancials`) and the dialog (`CampaignPayouts`); the RPC re-computes authoritatively. **Terminology unified (this pass):** the dashboard said "Available Balance"/"Mavjud balans"/"Доступный баланс" while the dialog said "Available to withdraw"/"Yechish uchun mavjud mablag'"/"Доступно для вывода" — the *same money under two labels*. The dashboard now uses the dialog's exact wording in all three languages, the dashboard is reordered to lead with **Available to withdraw** then its breakdown, and the redundant "Remaining balance" dashboard row (which equalled Available to withdraw) was removed. No fee/commission/ledger calculation changed.
...
- **Recommendation (not shipped — needs live-DB coordination):** presenting "Available to withdraw" as the **net** (9,600 for a 10,000 balance) so the creator requests/receives the same number is a *net-request* fee model — economically identical, but it requires a DB migration changing `create_payout_request` to store `payout = net` (`commission = round(net/24)`) applied **atomically** with a client change. It is deliberately not deployed here: without applying the migration first, a net-requesting client against the still-gross RPC would credit creators the wrong amount — the exact inconsistency this task guards against.
